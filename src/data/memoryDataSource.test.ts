...
test('Deve ser possível cadastrar uma pessoa', async () => {
  ...
  const person = await memoryDataSource.insertPersonRegistry({ id: 'aaaaaaaaa', name: 'willyam', password: 'willyam', email: 'willyampimenteldev@gmail.com', age: 22 })
...
test('Deve ser possível buscar uma pessoa cadastrada', async () => {
  const memoryDataSource = new MemoryDataSource()
  const person = await memoryDataSource.insertPersonRegistry({ id: 'aaaaaaaaa', name: 'willyam', password: 'willyam', email: 'willyampimenteldev@gmail.com', age: 22 })
  const fetchedPerson = await memoryDataSource.fetchPersonRegistryBy('id', 'aaaaaaaaa') as Person
...
test('Deve ser possível atualizar uma pessoa cadastrada', async () => {
  const memoryDataSource = new MemoryDataSource()
  await memoryDataSource.insertPersonRegistry({ id: 'aaaaaaaaa', name: 'willyam', password: 'willyam', email: 'willyampimenteldev@gmail.com', age: 22 })
...
test('Deve ser possível deletar uma pessoa cadastrada', async () => {
  const memoryDataSource = new MemoryDataSource()
  await memoryDataSource.insertPersonRegistry({ id: 'aaaaaaaaa', name: 'willyam', password: 'willyam', email: 'willyampimenteldev@gmail.com', age: 22 })
  const deletedPerson = await memoryDataSource.deletePersonRegistryBy('id', 'aaaaaaaaa') as Person
