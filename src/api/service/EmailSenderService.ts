...
class EmailSenderService implements IEmailSenderService {
  ...
  async sendPasswordChangeEmail (person: Person): Promise<void> {
    this.email.send(person.email, 'Troca de senha', `Para trocar a sua senha utilize a seguinte rota: localhost:4000/api/person/${person.id} com o verbo PATCH. Para tal, use o Insomnia ou algum cliente de requisições de seu gosto.
