...
test('Deve ser possível cadastrar uma pessoa', async () => {
  ...
  const person = await personService.create('willyam', 'willyam', 'willyampimenteldev@gmail.com', 22)
...
test('Deve ser possível buscar uma lista com as pessoas cadastrada', async () => {
  ...
  const nameNormalizer = new NameNormalizer()
  const personService = new PersonService(personRepository, idGenerator, nameNormalizer)
  const person = await personService.create('willya', 'willyam', 'willyampimenteldev@gmail.com', 22)
...
test('Deve ser possível atualizar uma pessoa cadastrada', async () => {
  ...
  const personService = new PersonService(personRepository, idGenerator, nameNormalizer)
  const person = await personService.create('willyam', 'willyam', 'willyampimenteldev@gmail.com', 22)
  const updatedPerson = await personService.update(person.id, { id: '', password: 'pimentel', name: 'pimentel', email: 'pimentelwillyamdev@gmail.com', age: 24 }) as Person
